...
function show(id){document.querySelectorAll('.page').forEach(p=>p.style.display='none');document.getElementById(id).style.display='block';renderCharts();}
...
// Paramètres
function saveParam(){
params={nom:nomSociete.value,adresse:adresseSociete.value,lieu:lieuSociete.value,tva:tva.value,dernier:dernierFacture.value};
...
new Chart(ctxCA,{type:"bar",data:{labels:ventes.map(v=>v.num),datasets:[{label:"Montant Vente",data:ventes.map(v=>v.montant),backgroundColor:"#
